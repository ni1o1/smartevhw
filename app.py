...
def UploadHeatmap():
    if request.method == 'POST':
        result = upload_file(request.files['file'], './Files/Heatmap')
        return result
...
@app.route('/GetHeatmapList', methods=['GET'])
def GetHeatmapList():
    if request.method == 'GET':
        returndata = get_file_info('./Files/Heatmap', 'csv')
...
def PreviewTraj():
    if request.method == 'GET':
        ...
        if filetype == 'gridscore':
            ...
            return returnresult
        if filetype == 'sitesuggest':
            areaclass = gpd.read_file('./Files/SiteSuggestOutput/'+filename)
            areaclass['count'] = areaclass['num']-areaclass['max_capacity']
            areaclass['max_count'] = areaclass['count'].max()
            returnresult = areaclass.to_json()
...
@app.route('/AreaClassV2', methods=['GET'])
def AreaClassV2():
    if request.method == 'GET':
        bins = int(request.args.get('bins'))
        filename = request.args.get('filename')
        ...
        taskname = request.args.get('taskname')
        taskname = taskname+str(bins)

        filename = r'./Files/GridScoreOutput/'+filename
        grid = gpd.read_file(filename)
        from Models.station_selection.main import area_classification_v2
        # 生成文件名
        with open(f'./Files/AreaClassOutput/{taskname}.generating', 'w') as f:
            f.write('This is a temp file')
        ...
        try:
            result = area_classification_v2(grid,bins)
            result['class'] = result['class'].astype(int)
            result.to_file(f'./Files/AreaClassOutput/{taskname}.json', driver='GeoJSON')
            returndata = {
                'status': 'success',
...
@app.route('/GridScore_diy', methods=['GET'])
def GridScore_diy():
    if request.method == 'GET':
        selectedDIYStation = request.args.get('selectedDIYStation')
        selectedScoreFile = request.args.get('selectedScoreFile')
        station = pd.read_csv(f'./Files/GridScoreInput/{selectedDIYStation}')
        grid_score = gpd.read_file(f'./Files/GridScoreOutput/{selectedScoreFile}')
        if ('lon' in station.columns) & ('lat' in station.columns):
            station['geometry'] = gpd.points_from_xy(station['lon'], station['lat'])
        elif ('stationLon' in station.columns) & ('stationLat' in station.columns):
            station['geometry'] = gpd.points_from_xy(station['stationLon'], station['stationLat'])
        station = gpd.GeoDataFrame(station, crs='EPSG:4326')
        station = gpd.sjoin(station, grid_score, how='inner', op='intersects')
        #station['geometry'] = station['geometry'].buffer(0.1)
...
@app.route('/KeylocFromHeatmap', methods=['GET'])
def KeylocFromHeatmap():
    if request.method == 'GET':
        heatmappath = request.args.get('heatmap')
        gridsize = int(request.args.get('gridsize'))
        taskname = request.args.get('taskname')

        import os
        if not os.path.exists(f'./Files/Keylocation/{taskname}.generating'):
            os.mkdir(f'./Files/Keylocation/{taskname}.generating')

        heatmap = pd.read_csv(f'./Files/Heatmap/{heatmappath}')
        
        from Models.hwo_predict.main import keyloc_from_heatmap
        h0_pob_dict,h2w_pob_dict,hw2o_pob_dict,params = keyloc_from_heatmap(heatmap,gridsize)
...
@app.route('/GetSiteSuggestOutputList', methods=['GET'])
def GetSiteSuggestOutputList():
    if request.method == 'GET':
        returndata = get_file_info('./Files/SiteSuggestOutput', 'json')
        return json.dumps(returndata)
    

@app.route('/SiteSuggestUrban', methods=['GET'])
def SiteSuggestUrban():
    if request.method == 'GET':
        filename = request.args.get('filename')
        taskname = request.args.get('taskname')
        ...
        # 城市场景
        station_info_path = f'./Files/SimUrbanResult/{filename}'
        from Models.station_selection.main import urban_capacity

        station=urban_capacity(station_info_path,stations_num=5000)

        station.to_file(f'./Files/SiteSuggestOutput/{taskname}.json', driver='GeoJSON')
        returndata = {
                            'status': 'success',
                            'message': '生成成功'
                        }
...
@app.route('/SiteSuggestHighway', methods=['GET'])
def SiteSuggestHighway():
    if request.method == 'GET':
        filename = request.args.get('filename')
        taskname = request.args.get('taskname')
        ...
        province = request.args.get('province')
        # 城市场景
        station_info_path = f'./Files/SimHighwayResult/{filename}'
        
        from Models.highway_sim.station_decesion import highway_chargenum

        station=highway_chargenum(province,station_info_path)
